...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://hl7.org/fhir" xmlns:lcg="http://www.lantanagroup.com" xmlns:xslt="http://www.w3.org/1999/XSL/Transform" xmlns:cda="urn:hl7-org:v3"
    xmlns:fhir="http://hl7.org/fhir" xmlns:uuid="java:java.util.UUID" version="2.0">

    <xsl:template match="/" mode="saxon" priority="-1">
        <xsl:apply-templates select="*" mode="add-uuids-saxon" />
    </xsl:template>

    <xsl:template match="cda:ClinicalDocument" mode="add-uuids-saxon">
        <xsl:copy>
            ...
            <xsl:apply-templates select="@* | node()" mode="add-uuids-saxon" />
            <lcg:binary>
                <xsl:attribute name="lcg:uuid">
                    <xsl:value-of select="uuid:randomUUID()" />
                </xsl:attribute>
    ...
    <xsl:template match="cda:*" mode="add-uuids-saxon">
        <xsl:copy>
            <xsl:if test="not(@lcg:uuid)">
                <xsl:attribute name="lcg:uuid">
                    <xsl:value-of select="uuid:randomUUID()" />
    ...
    <xsl:template match="@* | node()" mode="add-uuids-saxon">
        <xsl:copy>
            <xsl:apply-templates select="@* | node()" mode="add-uuids-saxon" />
        </xsl:copy>
    </xsl:template>
