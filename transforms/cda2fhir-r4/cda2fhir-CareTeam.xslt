        ...
        <participant>
          ...
          <xsl:choose>
            ...
            <xsl:otherwise>
              <role>
                <coding>
                  <!-- 20260727 Claude: Fix - system was the ValueSet URL (v3-xServiceEventPerformer); PPRF/PRF/SPRF are codes from the
                       v3-ParticipationType code system and Coding.system must be the code system URL (matches create-care-team below) -->
                  <system value="http://terminology.hl7.org/CodeSystem/v3-ParticipationType" />
                  <!-- 20260727 Claude: Fix - context node here is the Care Team Member Act (see for-each above), which has no @typeCode;
                       the typeCode is on the performer participation, so read cda:performer/@typeCode -->
                  <code value="{cda:performer/@typeCode}" />
                  <xsl:choose>
                    <xsl:when test="cda:performer/@typeCode = 'PPRF'">
                      <display value="primary performer" />
                    </xsl:when>
                    <xsl:when test="cda:performer/@typeCode = 'PRF'">
                      <display value="performer" />
                    </xsl:when>
                    <xsl:when test="cda:performer/@typeCode = 'SPRF'">
          ...
          <!-- 20260727 Claude: Fix - test path was cda:performer/cda:representedOrganization (missing the assignedEntity step,
               so it never matched and onBehalfOf was never emitted); now tests the same path that is selected -->
          <xsl:if test="cda:performer/cda:assignedEntity/cda:representedOrganization">
            <onBehalfOf>
