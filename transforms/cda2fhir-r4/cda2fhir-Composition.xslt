...
<xsl:stylesheet xmlns="http://hl7.org/fhir" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cda="urn:hl7-org:v3" xmlns:fhir="http://hl7.org/fhir" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <xsl:template match="cda:ClinicalDocument" mode="bundle-entry">
    ...
    <xsl:apply-templates select="//cda:relatedDocument[@typeCode = 'XFRM']" mode="bundle-entry" />
    <!-- Create a DocumentReference for the CDA document that is being transformed so we can store the setId and versionNumber-->
    <xsl:apply-templates select="//cda:ClinicalDocument" mode="manual-bundle-entry" />
  ...
  <xsl:template match="cda:ClinicalDocument">
    ...
    <Composition>
      ...
      <xsl:choose>
        <xsl:when test="(/cda:ClinicalDocument[cda:templateId/@root = '2.16.840.1.113883.10.20.15.2'] or /cda:ClinicalDocument[cda:templateId/@root = '2.16.840.1.113883.10.20.15.2.1.2']) and not(cda:versionNumber/@value)">
      ...
      <!--<!-\- 20260423 SG: Added code to transform already exising relatesTo data element -\->
      <!-\- relatesTo:sliceReplaced - Document or Composition that this Composition trasnforms 
                    Bundle.identifier = ClinicalDocument.id
                    Composition.identifier is equivalent to ClinicalDocument.setId in CDA
                    Composition.versionNumber = ClinicalDocument.versionNumber in CDA
                    ClinicalDocument.id is globally unique, can get versionNumber and setId from this globally unique id
            -\->
      <xsl:for-each select="cda:relatedDocument[@typeCode = 'XFRM']">
        <relatesTo>
          <code value="transforms" />
          <xsl:apply-templates select="." mode="reference">
            <xsl:with-param name="pElementName">targetReference</xsl:with-param>
          </xsl:apply-templates>
        </relatesTo>
      </xsl:for-each>-->
      ...
      <!--<xsl:for-each select="cda:relatedDocument[@typeCode = 'XFRM']">
        <relatesTo>
          <code value="transforms" />
          <reference>
            <xsl:apply-templates select="cda:parentDocument" />
          </reference>
          <!-\-<xsl:apply-templates select="cda:parentDocument/cda:id">
                <xsl:with-param name="pElementName">targetIdentifier</xsl:with-param>
              </xsl:apply-templates>-\->
  ...
  <xsl:template match="cda:section">
    ...
    <xsl:choose>
      ...
      <xsl:when test="($gvCurrentIg = 'eICR' and @nullFlavor = 'NI' and not(cda:entry)) and (cda:templateId/@root = '2.16.840.1.113883.10.20.22.2.10' or cda:templateId/@root = '2.16.840.1.113883.10.20.22.2.2.1' or cda:templateId/@root = '2.16.840.1.113883.10.20.22.2.7.1')" />
        ...
        <section>
          ...
          <xsl:choose>
            <xsl:when test="cda:entry">
              ...
              <xsl:for-each select="cda:entry[cda:*[not(cda:templateId[key('templates-to-suppress-key', @root)])][not(cda:code/@code = '8462-4')]]">
          ...
          <!-- get triggers lower in the hierarchy skip wrappers like concern etc.-->
          <xsl:for-each
            select="descendant::cda:entryRelationship[descendant::cda:*/descendant::cda:*[3]]
            ...
            [not(preceding-sibling::cda:templateId[@root = '2.16.840.1.113883.10.20.22.4.33'])]
            ...
            [not(preceding-sibling::cda:templateId[@root = '2.16.840.1.113883.10.20.22.4.36'])]
