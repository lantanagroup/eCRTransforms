...
<xsl:stylesheet xmlns="http://hl7.org/fhir" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="cda:act[cda:templateId[@root = '2.16.840.1.113883.10.20.22.4.60']]"
        mode="bundle-entry">

        <xsl:for-each
            select="cda:entryRelationship/cda:act[cda:templateId[@root = '2.16.840.1.113883.10.20.22.4.61']]">
            <xsl:call-template name="create-bundle-entry"/>
            <xsl:for-each
                select="cda:performer[cda:templateId[@root = '2.16.840.1.113883.10.20.22.4.87']]/cda:assignedEntity[cda:code[@code = 'PAYOR']]">
    ...
    <xsl:template match="cda:act[cda:templateId[@root = '2.16.840.1.113883.10.20.22.4.61']]">
        <Coverage>
                    ...
                    <meta>
                        <profile>
                            <xsl:attribute name="value"
                                select="'http://hl7.org/fhir/us/davinci-hrex/StructureDefinition/hrex-coverage'"
                            />
                        </profile>
                    </meta>
                </xsl:when>
            ...
            <xsl:choose>
                <xsl:when test="cda:participant[@typeCode = 'COV']">
                    <xsl:choose>
                        ...
                        <xsl:when
                            test="cda:participant[@typeCode = 'COV']/cda:participantRole[@classCode = 'PAT']">
                            <subscriberId>
                                <xsl:choose>
                                    <xsl:when
                                        test="cda:participant[@typeCode = 'COV']/cda:participantRole[@classCode = 'PAT']/cda:id/@nullFlavor">
                                        <xsl:attribute name="value" select="'NI'"/>
                                    </xsl:when>
                                    <xsl:when
                                        test="cda:participant[@typeCode = 'COV']/cda:participantRole[@classCode = 'PAT']/cda:id/@extension">
                                        <xsl:variable name="vTemp"
                                            select="cda:participant[@typeCode = 'COV']/cda:participantRole[@classCode = 'PAT']/cda:id/@extension"/>
                ...
                <xsl:when test="cda:participant[@typeCode = 'COV']">
                    <xsl:choose>
                        <xsl:when
                            test="cda:participant[@typeCode = 'COV']/cda:participantRole[@classCode = 'PAT']">
                            <xsl:apply-templates
                                select="cda:participant[@typeCode = 'COV']/cda:participantRole[@classCode = 'PAT']/cda:code">
            ...
            <xsl:choose>
                <xsl:when
                    test="cda:performer[cda:templateId/@root = '2.16.840.1.113883.10.20.22.4.87']/cda:time">
                    <xsl:apply-templates
                        select="cda:performer[cda:templateId/@root = '2.16.840.1.113883.10.20.22.4.87']/cda:time"
                    />
                </xsl:when>
            </xsl:choose>
    ...
    <xsl:template
        match="cda:performer[cda:templateId[@root = '2.16.840.1.113883.10.20.22.4.87']]/cda:assignedEntity[cda:code[@code = 'PAYOR']]"
        mode="payor">
        <Organization>
            <xsl:apply-templates select="cda:id"/>
            <active value="true"/>
