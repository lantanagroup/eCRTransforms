...
<xsl:stylesheet xmlns="http://hl7.org/fhir" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:lcg="http://www.lantanagroup.com"
  ...
  <xsl:template match="cda:assignedAuthor[cda:assignedAuthoringDevice]" mode="reference">
    ...
    <xsl:if test="not(@nullFlavor)">
      ...
      <xsl:element name="{$pElementName}">
        <xsl:attribute name="value">urn:uuid:<xsl:value-of
            select="cda:assignedAuthoringDevice/@lcg:uuid"/></xsl:attribute>
      </xsl:element>
    </xsl:if>
  </xsl:template>
